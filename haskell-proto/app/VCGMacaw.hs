...
runMStateM :: MState -> MStateM () -> [Event]
runMStateM s (MStateM f) = runReader (runContT f (\() -> pure [])) s
    ...
    ReadMem addr tp -> do
      ...
      case memEventInfo of
        Ann.BinaryOnlyAccess -> do
          addEvent $ MCOnlyStackReadEvent addrTerm tp (smtLocalVar aid)
        Ann.JointStackAccess aname -> do
          addEvent $ JointStackReadEvent addrTerm tp (smtLocalVar aid) aname
        Ann.HeapAccess -> do
          addEvent $ HeapReadEvent addrTerm tp (smtLocalVar aid)
...
blockEvents :: Map (MemSegmentOff 64) Ann.BlockEventInfo
            ...
            -> Integer
            -> [Event]
blockEvents evtMap addr sz = runST $ do
  ...
  case mBlock of
    ...
    Right b -> do
      ...
      pure $! runMStateM ms0 (block2SMT b)
