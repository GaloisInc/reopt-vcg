  ...
  definst "vpmovzxbq" $ do
    ...
    instr_pat $ fun (mem_0 : Mem) (ymm_1 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let v_10 <- eval (concat (expression.bv_nat 56 0) v_9);
      let v_11 <- eval (concat v_5 v_10);
      let v_12 <- eval (concat (expression.bv_nat 56 0) v_11);
      let v_13 <- eval (concat v_4 v_12);
      let v_14 <- eval (concat (expression.bv_nat 56 0) v_13);
    ...
    instr_pat $ fun (xmm_0 : reg (bv 128)) (ymm_1 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let v_9 <- eval (concat (expression.bv_nat 56 0) v_8);
      let v_10 <- eval (concat v_4 v_9);
      let v_11 <- eval (concat (expression.bv_nat 56 0) v_10);
      let v_12 <- eval (concat v_3 v_11);
      let v_13 <- eval (concat (expression.bv_nat 56 0) v_12);
