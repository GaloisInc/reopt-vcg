  ...
  definst "shrq" $ do
    instr_pat $ fun (_ : clReg) (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let (v_15 : expression (bv 8)) <- eval (extract v_8 56 64);
      let v_16 <- getRegister of;
      let v_17 <- getRegister cf;
      let v_18 <- getRegister af;
      setRegister af (mux v_11 v_18 undef);
      setRegister cf (mux (uge v_6 (expression.bv_nat 8 64)) undef (mux v_11 v_17 (isBitSet v_8 64)));
    ...
    instr_pat $ fun (_ : clReg) (r64_0 : reg (bv 64)) =>
     let action : semantics Unit := do
      ...
      let (v_13 : expression (bv 8)) <- eval (extract v_9 56 64);
      ...
      setRegister cf (mux (uge v_3 (expression.bv_nat 8 64)) undef (mux v_4 v_15 (isBitSet v_9 64)));
    ...
    instr_pat $ fun (imm_0 : imm int) (mem_1 : Mem) =>
     let action : semantics Unit := do
      ...
      let (v_14 : expression (bv 8)) <- eval (extract v_7 56 64);
      let v_15 <- getRegister of;
      let v_16 <- getRegister cf;
      let v_17 <- getRegister af;
      setRegister af (mux v_10 v_17 undef);
      setRegister cf (mux (uge v_5 (expression.bv_nat 8 64)) undef (mux v_10 v_16 (isBitSet v_7 64)));
    ...
    instr_pat $ fun (imm_0 : imm int) (r64_1 : reg (bv 64)) =>
     let action : semantics Unit := do
      ...
      let (v_12 : expression (bv 8)) <- eval (extract v_8 56 64);
      ...
      setRegister cf (mux (uge v_2 (expression.bv_nat 8 64)) undef (mux v_3 v_14 (isBitSet v_8 64)));
    ...
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_2 <- load v_1 8;
      let v_3 <- eval (concat v_2 (expression.bv_nat 1 0));
      let v_4 <- eval (lshr v_3 (expression.bv_nat 65 1));
      let (v_5 : expression (bv 64)) <- eval (extract v_4 0 64);
      store v_1 v_5 8;
      let (v_7 : expression (bv 8)) <- eval (extract v_4 56 64);
      setRegister af undef;
      setRegister cf (isBitSet v_4 64);
    ...
    instr_pat $ fun (r64_0 : reg (bv 64)) =>
     let action : semantics Unit := do
      ...
      let v_3 <- eval (lshr v_2 (expression.bv_nat 65 1));
