  ...
  definst "rclb" $ do
    ...
    instr_pat $ fun (_ : clReg) (rh_0 : reg (bv 8)) =>
     let action : semantics Unit := do
      ...
      let (v_12 : expression (bv 8)) <- eval (extract v_9 1 9);
      setRegister (lhs.of_reg rh_0) v_12;
      setRegister cf v_10;
    ...
    instr_pat $ fun (imm_0 : imm int) (rh_1 : reg (bv 8)) =>
     let action : semantics Unit := do
      ...
      let (v_11 : expression (bv 8)) <- eval (extract v_8 1 9);
      setRegister (lhs.of_reg rh_1) v_11;
      setRegister cf v_9;
