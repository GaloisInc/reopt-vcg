  ...
  definst "vpslldq" $ do
    instr_pat $ fun (imm_0 : imm int) (xmm_1 : reg (bv 128)) (xmm_2 : reg (bv 128)) =>
     let action : semantics Unit := do
      ...
      let (v_7 : expression (bv 128)) <- eval (extract (shl v_3 v_6) 0 128);
    ...
    instr_pat $ fun (imm_0 : imm int) (ymm_1 : reg (bv 256)) (ymm_2 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let (v_8 : expression (bv 128)) <- eval (extract (shl v_4 v_7) 0 128);
      let (v_9 : expression (bv 128)) <- eval (extract v_3 128 256);
      let (v_10 : expression (bv 128)) <- eval (extract (shl v_9 v_7) 0 128);
