  ...
  definst "negw" $ do
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_2 <- load v_1 2;
      let v_3 <- eval (add (expression.bv_nat 16 1) (bv_xor v_2 (expression.bv_nat 16 65535)));
      store v_1 v_3 2;
      let v_5 <- eval (isBitSet v_3 0);
      let (v_6 : expression (bv 8)) <- eval (extract v_3 8 16);
    ...
    instr_pat $ fun (r16_0 : reg (bv 16)) =>
     let action : semantics Unit := do
      ...
      let v_2 <- eval (add (expression.bv_nat 16 1) (bv_xor v_1 (expression.bv_nat 16 65535)));
