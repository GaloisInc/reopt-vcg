def sall : instruction :=
  definst "sall" $ do
    ...
    pattern fun (mem_0 : Mem) => do
      v_1 <- evaluateAddress mem_0;
      v_2 <- load v_1 4;
      (v_3 : expression (bv 33)) <- eval (extract (shl (concat (expression.bv_nat 1 0) v_2) (expression.bv_nat 33 1)) 0 33);
      (v_4 : expression (bv 32)) <- eval (extract v_3 1 33);
      store v_1 v_4 4;
      v_6 <- eval (isBitSet v_3 1);
      (v_7 : expression (bv 8)) <- eval (extract v_3 25 33);
    ...
    pattern fun (r32_0 : reg (bv 32)) => do
      ...
      (v_2 : expression (bv 33)) <- eval (extract (shl (concat (expression.bv_nat 1 0) v_1) (expression.bv_nat 33 1)) 0 33);
      (v_3 : expression (bv 32)) <- eval (extract v_2 1 33);
      v_4 <- eval (isBitSet v_2 1);
      (v_5 : expression (bv 8)) <- eval (extract v_2 25 33);
