  ...
  definst "sall" $ do
    instr_pat $ fun (_ : clReg) (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let (v_8 : expression (bv 33)) <- eval (extract (shl v_3 v_7) 0 33);
      ...
      let (v_16 : expression (bv 8)) <- eval (extract v_8 25 33);
      let v_17 <- getRegister cf;
      let v_18 <- eval (mux (uge v_6 (expression.bv_nat 8 32)) undef (mux v_11 v_17 (isBitSet v_8 0)));
    ...
    instr_pat $ fun (_ : clReg) (r32_0 : reg (bv 32)) =>
     let action : semantics Unit := do
      ...
      let (v_9 : expression (bv 33)) <- eval (extract (shl v_7 v_8) 0 33);
      let (v_10 : expression (bv 32)) <- eval (extract v_9 1 33);
      let v_11 <- getRegister sf;
      let v_12 <- eval (isBitSet v_9 1);
      let v_13 <- getRegister pf;
      let (v_14 : expression (bv 8)) <- eval (extract v_9 25 33);
      let v_15 <- getRegister cf;
      let v_16 <- eval (mux (uge v_3 (expression.bv_nat 8 32)) undef (mux v_4 v_15 (isBitSet v_9 0)));
    ...
    instr_pat $ fun (imm_0 : imm int) (mem_1 : Mem) =>
     let action : semantics Unit := do
      ...
      let (v_7 : expression (bv 33)) <- eval (extract (shl v_4 v_6) 0 33);
      ...
      let (v_15 : expression (bv 8)) <- eval (extract v_7 25 33);
      let v_16 <- getRegister cf;
      let v_17 <- eval (mux (uge v_5 (expression.bv_nat 8 32)) undef (mux v_10 v_16 (isBitSet v_7 0)));
    ...
    instr_pat $ fun (imm_0 : imm int) (r32_1 : reg (bv 32)) =>
     let action : semantics Unit := do
      ...
      let (v_8 : expression (bv 33)) <- eval (extract (shl v_6 v_7) 0 33);
      let (v_9 : expression (bv 32)) <- eval (extract v_8 1 33);
      let v_10 <- getRegister sf;
      let v_11 <- eval (isBitSet v_8 1);
      let v_12 <- getRegister pf;
      let (v_13 : expression (bv 8)) <- eval (extract v_8 25 33);
      let v_14 <- getRegister cf;
      let v_15 <- eval (mux (uge v_2 (expression.bv_nat 8 32)) undef (mux v_3 v_14 (isBitSet v_8 0)));
    ...
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_2 <- load v_1 4;
      let v_3 <- eval (concat (expression.bv_nat 1 0) v_2);
      let (v_4 : expression (bv 33)) <- eval (extract (shl v_3 (expression.bv_nat 33 1)) 0 33);
      let (v_5 : expression (bv 32)) <- eval (extract v_4 1 33);
      store v_1 v_5 4;
      let v_7 <- eval (isBitSet v_4 1);
      let (v_8 : expression (bv 8)) <- eval (extract v_4 25 33);
    ...
    instr_pat $ fun (r32_0 : reg (bv 32)) =>
     let action : semantics Unit := do
      ...
      let (v_3 : expression (bv 33)) <- eval (extract (shl v_2 (expression.bv_nat 33 1)) 0 33);
      let (v_4 : expression (bv 32)) <- eval (extract v_3 1 33);
      let v_5 <- eval (isBitSet v_3 1);
      let (v_6 : expression (bv 8)) <- eval (extract v_3 25 33);
