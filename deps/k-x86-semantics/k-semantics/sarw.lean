  ...
  definst "sarw" $ do
    instr_pat $ fun (_ : clReg) (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      setRegister cf (mux v_11 v_17 (isBitSet v_8 16));
    ...
    instr_pat $ fun (_ : clReg) (r16_0 : reg (bv 16)) =>
     let action : semantics Unit := do
      ...
      setRegister cf (mux v_4 v_15 (isBitSet v_9 16));
    ...
    instr_pat $ fun (imm_0 : imm int) (mem_1 : Mem) =>
     let action : semantics Unit := do
      ...
      setRegister cf (mux v_10 v_16 (isBitSet v_7 16));
    ...
    instr_pat $ fun (imm_0 : imm int) (r16_1 : reg (bv 16)) =>
     let action : semantics Unit := do
      ...
      setRegister cf (mux v_3 v_14 (isBitSet v_8 16));
    ...
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_2 <- load v_1 2;
      let v_3 <- eval (concat v_2 (expression.bv_nat 1 0));
      let v_4 <- eval (ashr v_3 (expression.bv_nat 17 1));
    ...
    instr_pat $ fun (r16_0 : reg (bv 16)) =>
     let action : semantics Unit := do
      ...
      let v_3 <- eval (ashr v_2 (expression.bv_nat 17 1));
