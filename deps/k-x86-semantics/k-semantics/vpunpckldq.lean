  ...
  definst "vpunpckldq" $ do
    ...
    instr_pat $ fun (mem_0 : Mem) (ymm_1 : reg (bv 256)) (ymm_2 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let (v_13 : expression (bv 32)) <- eval (extract v_6 192 224);
      let v_14 <- eval (concat v_12 v_13);
      let (v_15 : expression (bv 32)) <- eval (extract v_4 224 256);
      let (v_16 : expression (bv 32)) <- eval (extract v_6 224 256);
    ...
    instr_pat $ fun (ymm_0 : reg (bv 256)) (ymm_1 : reg (bv 256)) (ymm_2 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let (v_14 : expression (bv 32)) <- eval (extract v_3 224 256);
      let (v_15 : expression (bv 32)) <- eval (extract v_5 224 256);
