  ...
  definst "vrsqrtps" $ do
    ...
    instr_pat $ fun (mem_0 : Mem) (ymm_1 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let v_13 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_12));
      let (v_14 : expression (bv 32)) <- eval (extract v_3 160 192);
      let v_15 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_14));
      let (v_16 : expression (bv 32)) <- eval (extract v_3 192 224);
      let v_17 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_16));
      let (v_18 : expression (bv 32)) <- eval (extract v_3 224 256);
      let v_19 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_18));
    ...
    instr_pat $ fun (ymm_0 : reg (bv 256)) (ymm_1 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let v_12 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_11));
      let (v_13 : expression (bv 32)) <- eval (extract v_2 160 192);
      let v_14 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_13));
      let (v_15 : expression (bv 32)) <- eval (extract v_2 192 224);
      let v_16 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_15));
      let (v_17 : expression (bv 32)) <- eval (extract v_2 224 256);
      let v_18 <- eval (bv_bitcast_to_fp float_class.fp32 (/- (_) -/ sqrt_single v_17));
