def testw : instruction :=
  definst "testw" $ do
    instr_pat $ fun (imm_0 : imm int) (mem_1 : Mem) =>
     let action : semantics Unit := do
      let v_2 <- evaluateAddress mem_1;
      let v_3 <- load v_2 2;
      let v_4 <- eval (bv_and v_3 (handleImmediateWithSignExtend imm_0 16 16));
    ...
    instr_pat $ fun (imm_0 : imm int) (r16_1 : reg (bv 16)) =>
     let action : semantics Unit := do
      ...
      let v_3 <- eval (bv_and v_2 (handleImmediateWithSignExtend imm_0 16 16));
