  ...
  definst "vfmadd213pd" $ do
    instr_pat $ fun (mem_0 : Mem) (xmm_1 : reg (bv 128)) (xmm_2 : reg (bv 128)) =>
     let action : semantics Unit := do
      ...
      let (v_16 : expression (bv 64)) <- eval (extract v_6 64 128);
      ...
      let v_21 <- eval (concat (fp_bitcast_to_bv (fp_sub (fp_mul v_5 v_8) v_13) 64) (fp_bitcast_to_bv (fp_sub (fp_mul v_15 v_17) v_20) 64));
    ...
    instr_pat $ fun (mem_0 : Mem) (ymm_1 : reg (bv 256)) (ymm_2 : reg (bv 256)) =>
     let action : semantics Unit := do
      ...
      let (v_14 : expression (bv 64)) <- eval (extract v_6 128 192);
      let (v_15 : expression (bv 64)) <- eval (extract v_8 128 192);
      let (v_16 : expression (bv 64)) <- eval (extract v_3 192 256);
      let (v_17 : expression (bv 64)) <- eval (extract v_6 192 256);
      ...
      let v_21 <- eval (concat (/- (_,_,_) -/ vfmadd132_double v_4 v_7 v_9) v_20);
