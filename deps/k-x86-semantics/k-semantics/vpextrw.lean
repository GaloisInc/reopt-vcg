  ...
  definst "vpextrw" $ do
    instr_pat $ fun (imm_0 : imm int) (xmm_1 : reg (bv 128)) (mem_2 : Mem) =>
     let action : semantics Unit := do
      ...
      let (v_5 : expression (bv 3)) <- eval (extract (handleImmediateWithSignExtend imm_0 8 8) 5 8);
      let v_6 <- eval (concat (expression.bv_nat 125 0) v_5);
      let (v_7 : expression (bv 128)) <- eval (extract (shl v_6 (expression.bv_nat 128 4)) 0 128);
      let (v_8 : expression (bv 16)) <- eval (extract (lshr v_4 v_7) 112 128);
      store v_3 v_8 2;
      pure ()
     action;
    instr_pat $ fun (imm_0 : imm int) (xmm_1 : reg (bv 128)) (r32_2 : reg (bv 32)) =>
     let action : semantics Unit := do
      ...
      let (v_4 : expression (bv 3)) <- eval (extract (handleImmediateWithSignExtend imm_0 8 8) 5 8);
      let v_5 <- eval (concat (expression.bv_nat 125 0) v_4);
      let (v_6 : expression (bv 128)) <- eval (extract (shl v_5 (expression.bv_nat 128 4)) 0 128);
      let (v_7 : expression (bv 16)) <- eval (extract (lshr v_3 v_6) 112 128);
