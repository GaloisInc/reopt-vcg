  ...
  definst "roll" $ do
    instr_pat $ fun (_ : clReg) (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_6 <- eval (concat (expression.bv_nat 24 0) v_5);
      let v_7 <- eval (rol v_2 v_6);
      store v_1 v_7 4;
      let v_9 <- eval (isBitSet v_7 31);
    ...
    instr_pat $ fun (imm_0 : imm int) (mem_1 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_5 <- eval (concat (expression.bv_nat 24 0) v_4);
      let v_6 <- eval (rol v_3 v_5);
      store v_2 v_6 4;
      let v_8 <- eval (isBitSet v_6 31);
    ...
    instr_pat $ fun (imm_0 : imm int) (r32_1 : reg (bv 32)) =>
     let action : semantics Unit := do
      ...
      let v_4 <- eval (concat (expression.bv_nat 24 0) v_2);
      let v_5 <- eval (rol v_3 v_4);
      let v_6 <- eval (isBitSet v_5 31);
    ...
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      let v_1 <- evaluateAddress mem_0;
      let v_2 <- load v_1 4;
      let v_3 <- eval (rol v_2 (expression.bv_nat 32 1));
      store v_1 v_3 4;
      let v_5 <- eval (isBitSet v_3 31);
      setRegister cf v_5;
