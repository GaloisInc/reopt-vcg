  ...
  definst "idivb" $ do
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_5 <- load v_4 1;
      let v_6 <- eval (concat v_2 (/- (_,_) -/  idiv_remainder_int8 v_3 v_5));
      let v_7 <- eval (concat v_6 (/- (_,_) -/  idiv_quotient_int8 v_3 v_5));
      setRegister rax v_7;
    ...
    instr_pat $ fun (rh_0 : reg (bv 8)) =>
     let action : semantics Unit := do
      ...
      let v_5 <- eval (concat v_2 (/- (_,_) -/  idiv_remainder_int8 v_3 v_4));
      let v_6 <- eval (concat v_5 (/- (_,_) -/  idiv_quotient_int8 v_3 v_4));
      setRegister rax v_6;
      setRegister af undef;
      setRegister cf undef;
      setRegister of undef;
