def decl : instruction :=
  definst "decl" $ do
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      let v_1 <- evaluateAddress mem_0;
      let v_2 <- load v_1 4;
      let v_3 <- eval (sub v_2 (expression.bv_nat 32 1));
      store v_1 v_3 4;
      ...
      setRegister of (bit_and (isBitSet v_2 0) (eq v_6 (expression.bv_nat 31 0)));
    ...
    instr_pat $ fun (r32_0 : reg (bv 32)) =>
     let action : semantics Unit := do
      ...
      let v_2 <- eval (sub v_1 (expression.bv_nat 32 1));
      ...
      setRegister of (bit_and (isBitSet v_1 0) (eq v_4 (expression.bv_nat 31 0)));
