  ...
  definst "divw" $ do
    instr_pat $ fun (mem_0 : Mem) =>
     let action : semantics Unit := do
      ...
      let v_8 <- load v_7 2;
      let v_9 <- eval (concat v_2 (/- (_,_) -/  div_remainder_int16 v_6 v_8));
      let (v_10 : expression (bv 48)) <- eval (extract v_4 0 48);
      let v_11 <- eval (concat v_10 (/- (_,_) -/  div_quotient_int16 v_6 v_8));
    ...
    instr_pat $ fun (r16_0 : reg (bv 16)) =>
     let action : semantics Unit := do
      ...
      let v_8 <- eval (concat v_2 (/- (_,_) -/  div_remainder_int16 v_6 v_7));
      let (v_9 : expression (bv 48)) <- eval (extract v_4 0 48);
      let v_10 <- eval (concat v_9 (/- (_,_) -/  div_quotient_int16 v_6 v_7));
