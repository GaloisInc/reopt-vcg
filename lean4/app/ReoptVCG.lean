...
match parseBlockAnn blockJson with
| Except.error errMsg => 
  ...
  | (firstBlock :: _) => match blockMap.find? firstBlock.label with
    ...
    | Option.some ab => match ab.annotation with
      ...
      | BlockAnn.reachable firstBlockAnn =>
        ...
        | Except.ok addr =>
          if (addr.toNat == firstBlockAnn.startAddr)
          then pure ab.label
          else moduleThrow $ 
            fnm ++ " annotation address listed as " 
                ++ (firstBlockAnn.startAddr.pp_hex_at_width 16)
                ++ "; symbol table however lists address as " ++ addr.pp_hex ++ ".");
-- Verify each block.
