...
axiom I_am_really_sorry : Π(P : Prop),  P 
...
def assert_bv {m} [Monad m] [MonadExcept String m] (nenv : nat_env) (tp : type) : m ℕ :=
  match tp with
...
axiom inject_ax1 : Π(rtp : gpreg_type), 64 - gpreg_type.width' rtp + gpreg_type.width' rtp ≤ 64
...
def print_regs (s : machine_state) : String :=
  let lines := List.zipWith (λn (r : bitvec 64), if r = 0 then "" else (n ++ ": " ++ r.pp_hex ++ ", ")) reg.r64_names s.gpregs.toList
  in String.join lines

def print_set_flags (s : machine_state) : String :=
  let lines := List.zipWith (λn (r : Bool), if r then n else "") reg.flag_names s.flags.toList
...
structure evaluator_state : Type :=
  ...
  (locals       : RBMap ℕ (Sigma (value nenv)) (λ(n n' : Nat), decide (n < n')))
...
instance (ε): MonadExcept ε (Except ε) := 
  { throw := λα e, Except.error e, catch := λα m f, match m with | (Except.error e) := f e | _ := m }

instance (ε) [Inhabited ε] : Alternative (Except ε) := 
  { failure := λα, Except.error (default ε)
  , orelse  := λα, MonadExcept.orelse }
...
theorem value.type_eval_is_id: Π{tp : type}, value nenv (eval_type nenv tp) = value nenv tp :=
  I_am_really_sorry _
...
def evaluator.read_memory_at (addr : machine_word) (n : ℕ) : evaluator os_state nenv (bitvec n) := do
    s <- get, 
...
def evaluator.map_machine_state (f : machine_state → machine_state) : evaluator os_state nenv Unit :=
  modify (λ(s : evaluator_state os_state nenv), { s with system_state := { s.system_state with machine_state := f s.system_state.machine_state } })

def evaluator.write_memory_at : Π{tp : type} (addr : machine_word) (bytes : value nenv tp)
  , evaluator os_state nenv Unit
  | (bv e) addr bytes :=
    let width := eval_nat_expr nenv e
    in (if H : width = 8 * (width / 8)
        then evaluator.map_machine_state os_state nenv (λ(s : machine_state), s.store_word addr (bitvec.cong H bytes))
        else throw "evaluator.write_memory_at: width not a multiple of 8")
  | _ _ _ := throw "evaluator.write_memory_at not a bv"

def evaluator.arg_at_idx (idx : ℕ) : evaluator os_state nenv (arg_value nenv) :=
  do s <- get,
...
def evaluator.local_at_idx (idx : ℕ) (tp : type) : evaluator os_state nenv (value nenv tp) :=
  do s <- get,
     (match s.locals.find idx with
     | (some (Sigma.mk tp' v)) := value.type_check nenv _ v tp
     | none     := throw "local_at_idx: no arg at idx")

theorem width_width' : Π(rtp : gpreg_type), eval_nat_expr nenv rtp.width = rtp.width' :=
  I_am_really_sorry _

def concrete_reg.set : Π{tp : type}, concrete_reg tp -> value nenv tp -> evaluator os_state nenv Unit
  | ._ (concrete_reg.gpreg idx rtp) b := 
    let b' : bitvec rtp.width' := Eq.rec b (width_width' nenv rtp)
    in evaluator.map_machine_state os_state nenv (machine_state.update_gpreg idx (reg.inject rtp b'))
  | ._ (concrete_reg.flagreg idx)   b := 
    evaluator.map_machine_state os_state nenv (machine_state.update_flag idx (λ_, b))
  
def concrete_reg.from_state : Π{tp : type}, concrete_reg tp -> machine_state 
  -> value nenv tp
  | _ (concrete_reg.gpreg idx rtp) s := 
    let v := reg.project rtp (s.get_gpreg idx) in
...
def arg_lval.to_value' {m} [Monad m] [MonadExcept String m] 
  (s : machine_state)
  : arg_lval -> Π(tp : type), m (value nenv tp)
...
def arg_lval.to_value : arg_lval -> Π(tp : type), evaluator os_state nenv (value nenv tp)
...
def arg_lval.set_value : arg_lval -> Π{tp : type}, value nenv tp 
  -> evaluator os_state nenv  Unit
...
def arg_value.to_value : arg_value nenv -> Π(tp : type), evaluator os_state nenv (value nenv tp)
...
def arg_value.set_value : arg_value nenv -> Π{tp : type}, value nenv tp -> evaluator os_state nenv Unit
  | (arg_value.lval _ l) tp v := arg_lval.set_value os_state nenv l v
  | _ _ _ := throw "arg_value.set_value: not an lvalue"

def addr.read : Π{tp : type}, addr tp -> evaluator os_state nenv (value nenv tp)
  | tp (addr.arg idx) := do 
...
def addr.set : Π{tp : type}, addr tp -> value nenv tp -> evaluator os_state nenv Unit
...
-- def tester1 : Π{tp : type}, value nenv tp -> value nenv tp -> Bool
--   | (bv _)     := λv1 v2, v1 = v2
--   | _ := λ_ _, true

-- def tester2 : Π{tp : type}, value nenv tp -> value nenv tp -> Bool
--   | (bv _)     := λv1 v2, v1 - v2 = 0
--   | _ := λ_ _, true

-- #print tester2._main

def value.partial_eq : Π{tp : type}, type.has_eq tp -> value nenv tp -> value nenv tp -> Bool
  ...
  | (vec _ tp) pf v1 v2 := 
     ...
     in (List.zip (Array.toList v1) (Array.toList v2)).all (λ(v : (value nenv tp × value nenv tp)), value.partial_eq pf' v.fst v.snd)
...
def prim.eval : Π{tp : type}, prim tp -> evaluator os_state nenv (value nenv tp)
  ...
  | ._ (prim.neq tp) := 
    if pf : type.has_eq tp 
    then pure (λv1 v2, not (value.partial_eq nenv pf v1 v2))
  ...
  -- `c` is false.
  | ._ (prim.mux tp) := pure (λb l r, if b then l else r)
  ...
  | ._ (prim.adc i)         := pure (λx y b, bitvec.add x (bitvec.add y (bit_to_bitvec _ b)))
  | ._ (prim.uadc_overflows i) := pure (λx y b, bitvec.ult (x + y + bit_to_bitvec (eval_nat_expr nenv i) b) x)
  | ._ (prim.sadc_overflows i) := pure (λx y b, bitvec.slt (x + y + bit_to_bitvec (eval_nat_expr nenv i) b) x)
  ...
  | ._ (prim.ssbb_overflows i) := 
    pure (λx y b, bitvec.slt x (x - y - bit_to_bitvec (eval_nat_expr nenv i) b))
  -- `(usbb_overflows i)` true if unsigned sub overflows,
  -- the bit is a borrow bit.
  | ._ (prim.usbb_overflows i) := pure (λx y b, bitvec.ult x (x - y - bit_to_bitvec (eval_nat_expr nenv i) b))
  ...
  | ._ (prim.ule i) := pure (λx y, bitvec.ule x y)
  | ._ (prim.ult i) := pure (λx y, bitvec.ult x y)
  ...
  | tp (prim.slice w u l) := do
       ...
       in let rewr : value nenv (bv (u + 1 - l)) = value nenv (bv (nat_expr.lit n)) := I_am_really_sorry _ 
       in pure (λx, Eq.recOn rewr.symm (f x))
       -- pure (begin
  ...
  | ._ (prim.shl i)    := pure (λx (y : bitvec 8), bitvec.shl x y.to_nat)
  ...
  | ._ (prim.shl_carry w) := pure (λc b (i : bitvec 8), 
       match i.to_nat with
       | Nat.zero        := c
   ...
   | ._ (prim.shr i) := pure (λx (y : bitvec 8), bitvec.ushr x y.to_nat)
   ...
   -- the the result is false.
  | ._ (prim.shr_carry w) := pure (λb c (i : bitvec 8), 
       match i.to_nat with
       | Nat.zero     := c
...
-- else ff 
   ...
   | ._ (prim.sar i) := pure (λx (y : bitvec 8), bitvec.sshr x y.to_nat)
   ...
   | ._ (prim.sar_carry w) := pure (λb c (i : bitvec 8), 
       match i.to_nat with
       | Nat.zero     := c
       | (Nat.succ n) := 
...
--                           else bitvec.msb b)
  ...
  | ._ (prim.even_parity i) := pure (λb, bitvec.parity b = false)
  ...
  | ._ (prim.pair_fst x y) := pure (λ(v : value nenv x × value nenv y), v.fst)
  -- Return second element of a pair.
  | ._ (prim.pair_snd x y) := pure (λ(v : value nenv x × value nenv y), v.snd)

def value.make_undef : Π(tp : type), value nenv tp 
...
def expression.eval : Π{tp : type}, expression tp -> evaluator os_state nenv (value nenv tp)
...
def evaluator.set_ip (new_ip : bitvec 64) : evaluator os_state nenv Unit :=
  evaluator.map_machine_state os_state nenv (λ(s : machine_state), { s with ip := new_ip })

def lhs.set : Π{tp : type}, lhs tp -> value nenv tp -> evaluator os_state nenv Unit
...
def lhs.read : Π{tp : type}, lhs tp -> evaluator os_state nenv (value nenv tp)
...
def read_cpuid : evaluator os_state nenv Unit :=
  ...
  let cpuid_values : RBMap ℕ (bitvec 32 × bitvec 32 × bitvec 32 × bitvec 32) (λx y, decide (x < y)) :=
    RBMap.fromList [ (0, (0xd, 0x756e6547, 0x6c65746e, 0x49656e69))
                    ...
                    ] (λx y, decide (x < y)) in -- FIXME: we need to look at rcx sometimes as well
...
def event.eval : event -> evaluator os_state nenv Unit
  | event.syscall :=
    adaptState (λ(s : evaluator_state os_state nenv), (s.system_state, s))
...
def pattern.eval (p : pattern) (e : environment nenv) (s : system_state os_state) 
  ...
  in do (_, s') <- evaluator.run os_state nenv (List.mmap (action.eval os_state os_transition nenv) p.actions >>= λ_, pure ()) st, pure s'.system_state
