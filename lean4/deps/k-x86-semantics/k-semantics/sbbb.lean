def sbbb : instruction :=
  definst "sbbb" $ do
    ...
    pattern fun (imm_0 : imm int) (rh_1 : reg (bv 8)) => do
      ...
      setRegister of (bit_and (eq v_10 (isBitSet v_5 0)) (notBool_ (eq v_10 v_8)));
    ...
    pattern fun (mem_0 : Mem) (rh_1 : reg (bv 8)) => do
      ...
      (v_10 : expression (bv 1)) <- eval (extract v_4 0 1);
      v_11 <- eval (eq (bv_xor v_10 (expression.bv_nat 1 1)) (expression.bv_nat 1 1));
      setRegister (lhs.of_reg rh_1) v_8;
      setRegister af (notBool_ (eq (isBitSet (bv_xor v_4 v_6) 3) (isBitSet v_7 4)));
      setRegister cf (isBitClear v_7 0);
      setRegister of (bit_and (eq v_11 (isBitSet v_6 0)) (notBool_ (eq v_11 v_9)));
      setRegister pf (parityFlag v_8);
      setRegister sf v_9;
      setRegister zf (zeroFlag v_8);
      pure ()
