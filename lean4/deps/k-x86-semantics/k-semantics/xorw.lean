def xorw1 : instruction :=
  definst "xorw" $ do
    pattern fun (v_2836 : imm int) ax => do
      v_5255 <- getRegister rax;
      v_5257 <- eval (handleImmediateWithSignExtend v_2836 16 16);
      ...
      pure ()
    pat_end;
    pattern fun (v_2849 : imm int) (v_2847 : reg (bv 16)) => do
      v_5329 <- getRegister v_2847;
      v_5331 <- eval (handleImmediateWithSignExtend v_2849 16 16);
      v_5334 <- eval (bv_xor v_5329 v_5331);
      setRegister (lhs.of_reg v_2847) v_5334;
      setRegister of (expression.bv_nat 1 0);
