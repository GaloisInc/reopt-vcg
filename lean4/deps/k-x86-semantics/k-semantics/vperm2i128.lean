def vperm2i128 : instruction :=
  definst "vperm2i128" $ do
    pattern fun (imm_0 : imm int) (mem_1 : Mem) (ymm_2 : reg (bv 256)) (ymm_3 : reg (bv 256)) => do
      ...
      (v_8 : expression (bv 128)) <- eval (extract v_6 0 128);
      v_9 <- evaluateAddress mem_1;
      v_10 <- load v_9 32;
      (v_11 : expression (bv 128)) <- eval (extract v_10 128 256);
      (v_12 : expression (bv 128)) <- eval (extract v_10 0 128);
      (v_13 : expression (bv 2)) <- eval (extract v_4 6 8);
    ...
    pattern fun (imm_0 : imm int) (ymm_1 : reg (bv 256)) (ymm_2 : reg (bv 256)) (ymm_3 : reg (bv 256)) => do
      ...
      (v_10 : expression (bv 128)) <- eval (extract v_9 128 256);
      (v_11 : expression (bv 128)) <- eval (extract v_9 0 128);
      (v_12 : expression (bv 2)) <- eval (extract v_4 6 8);
