def rolw : instruction :=
  definst "rolw" $ do
    pattern fun (_ : clReg) (mem_0 : Mem) => do
      ...
      v_6 <- eval (rol v_2 (concat (expression.bv_nat 8 0) v_5));
      store v_1 v_6 2;
      v_8 <- eval (isBitSet v_6 15);
    ...
    pattern fun (_ : clReg) (r16_0 : reg (bv 16)) => do
      ...
      v_5 <- eval (rol v_4 (concat (expression.bv_nat 8 0) v_3));
      v_6 <- eval (isBitSet v_5 15);
