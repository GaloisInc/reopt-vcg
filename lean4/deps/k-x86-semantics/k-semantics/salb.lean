def salb : instruction :=
  definst "salb" $ do
    ...
    pattern fun (mem_0 : Mem) => do
      v_1 <- evaluateAddress mem_0;
      v_2 <- load v_1 1;
      (v_3 : expression (bv 9)) <- eval (extract (shl (concat (expression.bv_nat 1 0) v_2) (expression.bv_nat 9 1)) 0 9);
    ...
    pattern fun (rh_0 : reg (bv 8)) => do
      ...
      (v_2 : expression (bv 9)) <- eval (extract (shl (concat (expression.bv_nat 1 0) v_1) (expression.bv_nat 9 1)) 0 9);
