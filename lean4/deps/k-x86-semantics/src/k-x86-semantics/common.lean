...
inductive type
| bv (w:Nat) : type
| int : type -- mainly for immediates
| bit : type
| float  : type
| double : type
| x86_80 : type
...
| flagreg (idx:Fin 32) : concrete_reg (bv 1)
...
| cat (i:Nat)(j:Nat) : prim (bv i .→ bv j .→ bv (i + j))
...
instance bv_to_x86_80  (w: Nat /- one_of [16,32] -/) : HasCoe (bv w) x86_80 := ⟨prim.bv_to_x86_80 w⟩
